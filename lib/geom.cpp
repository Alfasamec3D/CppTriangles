...
bool operator==(const Point point1, const Point point2) {
  return eps_eq(point1.x(), point2.x()) && eps_eq(point1.y(), point2.y()) &&
         eps_eq(point1.z(), point2.z());
}
bool operator==(const Segment segment1, const Segment segment2) {
  return ((segment1.m_edges[0] == segment2.m_edges[0]) &&
          (segment1.m_edges[1] == segment2.m_edges[1])) ||
         ((segment1.m_edges[0] == segment2.m_edges[1]) &&
          (segment1.m_edges[1] == segment2.m_edges[0]));
}
...
bool operator==(const Vector vector1, const Vector vector2) {
  return eps_eq(vector1.x(), vector2.x()) && eps_eq(vector1.y(), vector2.y()) &&
         eps_eq(vector1.z(), vector2.z());
}
...
Segment Triangle::segm() const {
  for (int i = 0; i < 3; i++) {
    if ((sqrdist(m_apexes[i], m_apexes[(i + 1) % 3]) >=
         sqrdist(m_apexes[(i + 1) % 3], m_apexes[(i + 2) % 3])) &&
        (sqrdist(m_apexes[i], m_apexes[(i + 1) % 3]) >=
         sqrdist(m_apexes[(i + 2) % 3], m_apexes[i]))) {
...
Point Triangle::minCorner() const {
  return {
      std::min(std::min(m_apexes[0].x(), m_apexes[1].x()), m_apexes[2].x()),
...
Point Triangle::maxCorner() const {
  return {
      std::max(std::max(m_apexes[0].x(), m_apexes[1].x()), m_apexes[2].x()),
...
Point intsec(const Segment segment, const Triangle triangle) {
  Vector temp = segment.m_edges[0] +
                segment.dir() *
                    scalprod({segment.m_edges[0], triangle.m_apexes[0]},
                             triangle.norm()) *
                    (1 / scalprod(segment.dir(), triangle.norm()));
...
bool compare(const Point point, const Segment segment) {
  return (eps_eq(std::pow(scalprod(Vector{segment.m_edges[0], point},
                                   Vector{segment.m_edges[1], point}),
                          2),
                 sqrdist(segment.m_edges[0], point) *
                     sqrdist(segment.m_edges[1], point)) &&
          (eps_sgn(scalprod(Vector{segment.m_edges[0], point},
                            Vector{segment.m_edges[1], point})) <= 0));
}
bool compare(const Segment segment1, const Segment segment2) {
  if (eps_sgn(scalprod({segment1.m_edges[0], segment2.m_edges[0]},
                       vectprod(segment1.dir(), segment2.dir()))) != 0) {
    return false;
  }
  Vector n1 =
      vectprod(segment1.dir(), vectprod(segment1.dir(), segment2.dir()));
  Vector n2 =
      vectprod(segment2.dir(), vectprod(segment1.dir(), segment2.dir()));
  ...
  return ((d11 != d12) && (d21 != d22)) ||
         ((d11 == 0) && (d12 == 0) &&
          ((compare(segment1.m_edges[0], segment2)) ||
           (compare(segment1.m_edges[1], segment2)) ||
           (compare(segment2.m_edges[0], segment1))));
}
bool compare(const Point point, const Triangle triangle) {
  ...
  return triangle.norm() ==
         Triangle{point, triangle.m_apexes[0], triangle.m_apexes[1]}.norm() *
                 eps_sgn(scalprod(
                     triangle.norm(),
                     Triangle{point, triangle.m_apexes[0], triangle.m_apexes[1]}
                         .norm())) +
             Triangle{point, triangle.m_apexes[1], triangle.m_apexes[2]}
                     .norm() *
                 eps_sgn(scalprod(
                     triangle.norm(),
                     Triangle{point, triangle.m_apexes[1], triangle.m_apexes[2]}
                         .norm())) +
             Triangle{point, triangle.m_apexes[2], triangle.m_apexes[0]}
                     .norm() *
                 eps_sgn(scalprod(
                     triangle.norm(),
                     Triangle{point, triangle.m_apexes[2], triangle.m_apexes[0]}
...
bool compare(const Segment segment, const Triangle triangle) {
  int d1 = eps_sgn(
      scalprod({triangle.m_apexes[0], segment.m_edges[0]}, triangle.norm()));
  int d2 = eps_sgn(
      scalprod({triangle.m_apexes[0], segment.m_edges[1]}, triangle.norm()));
  if ((d1 + d2 == 2) || (d1 + d2 == -2)) {
    return false;
  } else if ((d1 == 0) && (d2 == 0)) {
...
bool compare(const Triangle triangle1, const Triangle triangle2) {
  ...
  if ((triangle1.actual() == 2) && (triangle2.actual() == 2)) {
    ...
    std::vector<int> d2(3);
    for (int i = 0; i < 3; i++) {
      d1[i] = eps_sgn(scalprod({triangle2.m_apexes[0], triangle1.m_apexes[i]},
                               triangle2.norm()));
    }
    for (int i = 0; i < 3; i++) {
      d2[i] = eps_sgn(scalprod({triangle1.m_apexes[0], triangle2.m_apexes[i]},
                               triangle1.norm()));
    }
    if (((d1[0] == d1[1]) && (d1[1] == d1[2]) && (d1[0] != 0)) ||
        ((d2[0] == d2[1]) && (d2[1] == d2[2]) && (d2[0] != 0))) {
      return false;
    }
    if ((d1[0] == 0) && (d1[1] == 0) && (d1[2] == 0)) {
      return compare(triangle1.m_apexes[0], triangle2) ||
             compare(triangle1.m_apexes[1], triangle2) ||
             compare(triangle1.m_apexes[2], triangle2) ||
             compare(triangle2.m_apexes[0], triangle1) ||
             compare(triangle2.m_apexes[1], triangle1) ||
             compare(triangle2.m_apexes[2], triangle1);
    }
    ...
    for (int i = 0; i < 3; i++) {
      ...
      if ((d1[i] == 0) && (d1[(i + 1) % 3] == -d1[(i + 2) % 3])) {
        k1 = 2;
        t11 = triangle1.m_apexes[i];
        segment11 = {triangle1.m_apexes[(i + 1) % 3],
                     triangle1.m_apexes[(i + 2) % 3]};
    ...
    for (int i = 0; i < 3; i++) {
      ...
      if ((d2[i] == 0) && (d2[(i + 1) % 3] == -d2[(i + 2) % 3])) {
        k2 = 2;
        t21 = triangle2.m_apexes[i];
        segment21 = {triangle2.m_apexes[(i + 1) % 3],
                     triangle2.m_apexes[(i + 2) % 3]};
...
void final() {
  ...
  for (int i = 0; i < N; ++i) {
    ...
    Box box(bg::model::point<double, 3, bg::cs::cartesian>(
                boundingBoxes[i].minCorner.x(), boundingBoxes[i].minCorner.y(),
                boundingBoxes[i].minCorner.z()),
            bg::model::point<double, 3, bg::cs::cartesian>(
                boundingBoxes[i].maxCorner.x(), boundingBoxes[i].maxCorner.y(),
                boundingBoxes[i].maxCorner.z()));
    rtree.insert({box, i});
  }
